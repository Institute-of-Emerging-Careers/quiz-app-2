    ...
    return new Promise(async (resolve)=>{
        ...
        assignments.forEach((assignment) => {
            ...
            if (assignment.Attempts.length > 0) {
                quiz_sections.forEach((section) => {
                    ...
                    assignment.Attempts.forEach((attempt) => {
                        ...
                        if (section.section_id == attempt.SectionId) {
                            const section_score =
                            attempt.Score == null ? 0 : attempt.Score.score;
                            const percentage_score = roundToTwoDecimalPlaces((section_score / section.maximum_score) * 100);
                ...
                analysis.percentage_ranges[(cur_student_percentage-(cur_student_percentage%10))/10]++;
            }
        });

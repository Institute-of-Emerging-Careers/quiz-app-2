...
$("#invite-link-creation-form").submit(function (e) {
  ...
  $.post("/quiz/create-invite", $("#invite-link-creation-form").serialize(), function (data) {
    link_creation_spinner.toggleClass("hidden-imp");
    ...
    if (data.success == false) {
      ...
      previous_links_table_body.append("<tr><td>" + linkToFullUrl(data.invite.link) + "</td><td></td><td class='text-blue-600'><a href='quiz/registrations/" + data.invite.link + "'>" + data.invite.registrations + " (View All)</a></td><td>" + moment(data.invite.createdAt).format("Do MMMM, YYYY") + "</td></tr>");
    }
  }, "json");
});

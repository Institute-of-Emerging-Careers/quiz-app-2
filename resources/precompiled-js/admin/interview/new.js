...
const Step2 = () => {
  ...
  useEffect(() => {
    fetch(`/admin/interview/interviewers/all/${interview_round_id}`).then(raw_response => {
      if (raw_response.ok) {
...
const Main = () => {
  ...
  const updateInterviewRoundTitle = e => {
    e.preventDefault();
    setLoadingName(true);
    fetch(`/admin/interview/update-round-title/${document.getElementById("interview-round-id-field").value}`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json"
      },
      ...
      if (response.ok) {
        ...
        alert(`Error changing interview round name. Response code ${response.status}.`);
      }
    }).catch(err => {
      console.log(err);
  ...
  return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("a", {
    ...
    className: "text-2xl"
  }, `${interviewRoundTitle} `, /*#__PURE__*/React.createElement("i", {

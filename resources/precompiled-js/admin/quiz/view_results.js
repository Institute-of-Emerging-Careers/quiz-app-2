...
const StudentsList = () => {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/quiz/${quiz_id}/results-data`).then(raw_data => {
      setLoading(false);
  ...
  function resetStudentAssignment(e) {
    ...
    if (x == "yes") {
      fetch(`/reset-assignment/student/${e.target.dataset.student_id}/quiz/${quiz_id}`).then(res => {
        if (res.ok) {
  ...
  function resetOneSection(student_id, quiz_id) {
    ...
    fetch(`/quiz/reset-section-attempt/${student_id}/${sections[choice].section_id}`).then(res => {
      if (res.ok) {
        alert("Section reset");
        setReloadResults(cur => !cur);

...
const App = () => {
  ...
  useEffect(() => {
    fetch(`/quiz/all-assignees/${document.getElementById("quiz-id-field").value}`).then(raw_response => {
      if (!raw_response.ok) {
  ...
  const displayApplicationRoundStudents = e => {
    ...
    console.log("hi");
    fetch(`/admin/application/all-applicants-and-quiz-assignments?application_round_id=${application_round_id}&quiz_id=${document.getElementById("quiz-id-field").value}`).then(raw_response => {
      if (raw_response.ok) {
        raw_response.json().then(response => {

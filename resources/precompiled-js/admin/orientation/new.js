...
const EmailForm = () => {
  ...
  return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("h2", {
    ...
    value: recipients.length > 0 ? `Sending to ${recipients[0].email}, and ${recipients.length - 1} others` : "No recipients",
    onChange: e => {
...
const NameForm = () => {
  ...
  useEffect(() => {
    if (window.location.href.split("/")[window.location.href.split("/").length - 2] == "new") {
      window.location = `/admin/orientation/edit/${document.getElementById("orientation-id-field").value}`;
    } else {
  ...
  const saveData = e => {
    ...
    fetch(`/admin/orientation/save/${document.getElementById("orientation-id-field").value}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
...
const OrientationDetailsForm = () => {
  ...
  const [meeting_data, setMeetingData] = meeting_data_object;
  useEffect(() => {
    fetch(`/admin/orientation/get-meeting-data/${orientation_id_value}`).then(resp => {
      if (resp.ok) {

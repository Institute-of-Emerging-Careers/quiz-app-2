...
const ErrorDisplay = ({ errorType, email }) => {
	return (
		<div className="flex items-center gap-x-2">
			...
			{errorType === ERROR_TYPE.EMAIL_EXISTS && (
				<p>
					The email you entered already exists in our database. It means you
					have already applied to a different IEC cohort before. But you entered
					a different CNIC number last time. Please use the same combination of
					email and CNIC as last time.
					<br />
					Or, if you think you accidentally entered the wrong CNIC number last
					time, you can{" "}
					<a
					...
					>
						click here to change your CNIC number
					</a>{" "}
					if you remember your password from last time
				</p>
			)}
			{errorType === ERROR_TYPE.CNIC_EXISTS && (
				<p>
					We already have this CNIC in our database. It means you have applied
					to IEC in the past, but you used a different email address the last
					time. The email address you used last time looked something like this:{" "}
					{email}.<br />
					If that email address was correct, then please use that same email
					address and cnic pair.
					<br />
			...
			{errorType === ERROR_TYPE.PASSWORD_TOO_SHORT && (
				<p>Password must be at least 8 characters long.</p>
			)}
			{errorType === ERROR_TYPE.PASSWORD_MISMATCH && (
				<p>
					Please write the same password both times. The two password fields do
					not match.
				</p>
			)}
		</div>
	...
	for (const fieldName in validationSchema) {
		...
		if (fieldValidation.min_length && formData.has(fieldName)) {
			...
			if (fieldValue.length < fieldValidation.min_length) {
				errors[
					fieldName
				] = `${fieldName} should be at least ${fieldValidation.min_length} characters long`
				error_exists = true
			}
		}
		...
		if (fieldValidation.max_length && formData.has(fieldName)) {
			...
			if (fieldValue.length > fieldValidation.max_length) {
				errors[
					fieldName
				] = `${fieldName} should be at most ${fieldValidation.max_length} characters long`
				error_exists = true
			}
		}
		...
		if (fieldValidation.is_int && formData.has(fieldName)) {
			...
			if (isNaN(fieldValue) || !Number.isInteger(fieldValue)) {
				...
				errors[
					fieldName
				] = `${fieldName} should be at least ${fieldValidation.min}`
				error_exists = true
			} else if (fieldValidation.max && fieldValue > fieldValidation.max) {
				errors[
					fieldName
				] = `${fieldName} should be at most ${fieldValidation.max}`
				error_exists = true
			}
		}
...
const App = () => {
	...
	useEffect(() => {
		if (password !== confirmPassword) setErrorType(ERROR_TYPE.PASSWORD_MISMATCH)
	...
	useEffect(async () => {
		...
		const response = await fetch(`/application/${application_round_id}/courses`)
		const data = await response.json()
		if (data.success) {
			setCourses(data.courses)
			validationSchema["course_interest"]["is_in"] = data.courses.map(
				...
				>
					...
					<div
						className={`flex flex-col ${
							status === STATUS_TYPES.JUST_OPENED ? "flex-col" : "md:flex-row"
					...
					>
						<div id="left" className="flex flex-col w-full basis-full gap-y-5">
							...
							<Input
								label={
									<span>
										<i className="far fa-address-card"></i> CNIC:
									</span>
								}
							...
							>
								...
								<div className="flex flex-col gap-1 w-full">
									...
									>
										...
										<option value="Bachelors (In process)">
											Bachelors (In process)
										</option>

										<option value="Bachelors (Completed)">
											Bachelors (Completed)
										</option>

										<option value="Diploma (In process)">
											Diploma (In process)
										</option>
										...
										<option value="Postgraduate (In process)">
											Postgraduate (In process)
										</option>

										<option value="Postgraduate (Completed)">
											Postgraduate (Completed)
										</option>
									</select>
								</div>

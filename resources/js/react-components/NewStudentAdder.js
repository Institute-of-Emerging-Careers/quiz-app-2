		...
		filtered_students
			...
			.forEach((student) => {
				setStudents((cur_students_array) => {
					const copy = cur_students_array.slice()
		...
		setFilteredStudents(
			students.filter(
				(student) =>
					...
					DateTime.fromISO(student.assignment_completed_date).startOf('day')
						.ts >=
						DateTime.fromFormat(filter_date, 'yyyy-MM-dd').startOf('day').ts &&
					((student.added &&
	...
	return (
		<div>
			...
			<div ref={section2}>
				<div className="grid grid-cols-6 items-center">
					...
					<div className="col-span-2">
						<label htmlFor="filter_min_score">
							Filter by Latest Submission Date:{' '}
						</label>
						<input
							type="date"
							min="0"
				...
				<p>
					<b>{filtered_students.length}</b> filtered students.
				</p>
					...
					<tbody>
						{filtered_students.map((student, filtered_student_index) => (
							<tr className="py-2" key={student.id}>
								...
								<td className="border px-4 py-2">
									{DateTime.fromISO(student.assignment_completed_date).toFormat(
										'dd LLL yyyy'
									)}
								</td>

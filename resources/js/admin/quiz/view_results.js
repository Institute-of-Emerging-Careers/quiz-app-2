...
const StudentsList = () => {
	...
	function resetStudentAssignment(e) {
		const x = prompt('Are you sure? Enter "yes" or "no".')
		if (x === 'yes') {
			fetch(
				...
				<div>
					<div className="mb-2 text-md grid gap-y-2 grid-cols-2 md:grid-cols-8 w-full justify-between justify-items-center items-center text-xs">
						...
						<div className="col-span-2 justify-self-center">
							...
							></input>{' '}
							<label>
								Show each section&apos;s details (time of submission, exact
								marks, duration)
							</label>
						</div>
						<div className="col-span-2 justify-self-center">
					...
					>
						<thead className="bg-iec-blue text-white w-full">
							<tr className="w-full header_row">
								...
								{show_section_details
									? [
											<th className="py-3 px-6" key={`${section.id}f`}>
						...
						<tbody className="w-full">
							{filtered_students.slice(0, num_rows_shown).map((student) => (
								...
								>
									...
									{show_student_personal_details
										? [
												<td className="py-3 px-6" key={`${section.id}g`}>
									...
									{student.sections.map((section) =>
										show_section_details ? (
											section.status === 'Attempted' ? (
												[
													<td className="py-3 px-6" key={`${section.id}i`}>
														{section.percentage_score}
													</td>,
													<td className="py-3 px-6" key={`${section.id}j`}>
														{section.section_score}
													</td>,
													<td className="py-3 px-6" key={`${section.id}k`}>
														{section.duration}
													</td>,
													<td className="py-3 px-6" key={`${section.id}l`}>
														{section.end_time}
													</td>,
												]
											) : (
												[
													<td className="py-3 px-6" key={`${section.id}m`}>
														Not Attempted
													</td>,
													<td className="py-3 px-6" key={`${section.id}n`}>
														Not Attempted
													</td>,
													<td className="py-3 px-6" key={`${section.id}n`}>
														N/A
									...
									{show_section_details
										? [
												<td className="py-3 px-6" key={`${section.id}p`}>
													{student.total_score}
												</td>,
										  ]
										: []}

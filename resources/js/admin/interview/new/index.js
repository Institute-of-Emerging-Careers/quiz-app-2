...
const Step1 = () => {
	...
	const [students] = students_object
...
const Step3 = () => {
	...
	const computeMatching = async (e) => {
		...
		try {
			...
			await fetch(
				`/admin/interview/${interview_round_id}/set-interview-duration`,
				{
					method: 'POST',
					headers: {
			...
			if (res.ok) {
				...
				const unique_interviewers = [
					...new Set(flattened_matching.map((item) => item.interviewer_email)),
				]

				unique_interviewers.map(async (interviewer_email) => {
					await fetch(
						`/admin/interview/${interview_round_id}/create-booking-slots`,
						{
							method: 'POST',
							headers: {
								'Content-Type': 'application/json',
	...
	useEffect(async () => {
		try {
			let response = await fetch(
				`/admin/interview/${interview_round_id}/get-student-scores`
			)
	...
	return (
		<div className="flex flex-col justify-center items-center mt-10">
			{students.length > 0 ? (
				...
				<div className="w-full bg-white flex items-center justify-center text-xl mt-20 rounded-md p-4 ">

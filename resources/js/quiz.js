...
const Option = (props) => {
	...
	function uploadFile(e) {
		const data = new FormData(fileUploadForm.current)
		setUploading(true)
...
const MCQ = (props) => {
	...
	const [editing_statement, setEditingStatement] = useState(statement === null)
	...
	function deleteQuestion(e) {
		setState((cur) => {
			...
			const passages_copy = obj.passages.slice()
...
const Passage = (props) => {
	...
	function handleCheckboxChange(e) {
		setState((cur) => {
			const obj = { ...cur }
			const mcqs_copy = obj.mcqs.slice()
			const passages_copy = obj.passages.slice()
...
const SectionHeader = (props) => {
	...
	const [timeOrNot, setTimeOrNot] = useState(
		state.mcqs[props.sectionIndex].time === 0
	)
	...
	const addNewMCQ = (type) => {
		...
		setState((cur) => {
			const obj = { ...cur }
			const copy = obj.mcqs.slice()
			const question_order = copy[props.sectionIndex].questions.push({
				passage: null,
				statement: null,
				questionOrder: null,
				type,

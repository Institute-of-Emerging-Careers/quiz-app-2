...
invite_url_field.addEventListener("keyup", (e) => {
  real_time_url.innerHTML = "Link URL Preview: <span class='text-blue-600'>" + linkToFullUrl(e.target.url) + "</span>";
});
  ...
  $.post(
    ...
    function (data) {
      ...
      if (data.success == false) {
        ...
        previous_links_table_body.append("<tr><td>" + linkToFullUrl(data.invite.link) + "</td><td>" + data.invite.registrations + "</td>" + moment(data.invite.createdAt).format("Do MMMM, YYYY") + "</tr>");
      }
    },
    "json"
  );

...
real_time_url.innerHTML =
	"Link URL Preview: <span class='text-blue-600'>" +
	linkToFullUrl(invite_url_field.value) +
	'</span>'
	...
	$.post(
		...
		function (data) {
			...
			if (data.success === false) {
				...
				previous_links_table_body.append(
					'<tr><td>' +
						linkToFullUrl(data.invite.link) +
						"</td><td></td><td class='text-blue-600'><a href='quiz/registrations/" +
						data.invite.link +
						"'>" +
						data.invite.registrations +
						' (View All)</a></td><td>' +
						moment(data.invite.createdAt).format('Do MMMM, YYYY') +
						'</td></tr>'
				)
			}
		},
...
function toggleReminderEmailSetting(dom_obj) {
	...
	const data_to_send = {
		current_reminder_setting,
		quiz_id,
	}

	$.post('/quiz/edit-reminder-setting', data_to_send, function (data) {
		if (data.success) {
			dom_obj.children[1].innerHTML = data.new_reminder_setting
				? 'Disable Reminder Emails'
				: 'Enable Reminder Emails'
			dom_obj.children[0].classList.remove(
				data.new_reminder_setting ? 'far' : 'fas'
			)
			dom_obj.children[0].classList.add(
				data.new_reminder_setting ? 'fas' : 'far'
			)

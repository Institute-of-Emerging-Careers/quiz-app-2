	...
	fetch('/application/change-cnic', { method: 'POST', body: data }).then(
		(res) => {
			if (res.ok) {
				...
				alert(
					'CNIC not changed. There is something wrong with your email and password. Contact mail@iec.org.pk.'
				)
			} else {
				submit_button.classList.remove('btn-warning')
				submit_button.classList.add('btn-primary')
				document.getElementById('submit-button').value = 'Try Again'
				alert(
					'CNIC not changed. Please enter a valid email address, CNIC, and password. Contact mail@iec.org.pk.'
				)
			}
		}
	)
